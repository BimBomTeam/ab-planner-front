...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final informatyka = programs.firstWhere(
        (p) => p.name.toLowerCase().contains('informatics') || 
               p.name.toLowerCase().contains('computer science') ||
               p.name.toLowerCase().contains('informatyka'),
        orElse: () => programs.first, // Fallback na pierwszy program jeśli nie znajdzie
      );
  ...
  Future<void> _loadSavedGroup(int groupId) async {
    try {
      ...
      print('Nie udało się załadować zapisanej grupy: $e');
    }
  }

  Future<void> _loadGroups() async {
    if (_informaticsProgram == null || _selectedYear == null || _selectedSpecialization == null) {
      return;
    }
    ...
    try {
      ...
      setState(() {
        _groups = groups;
        if (_selectedGroup != null && !groups.any((g) => g.id == _selectedGroup!.id)) {
          _selectedGroup = null;
        }
      });
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Błąd ładowania grup: $e')),
        );
      }
    }
  }
  ...
  Future<void> _saveSettings() async {
    if (_selectedGroup == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Wybierz grupę')),
      );
      return;
    }
                ...
                children: [
                  ...
                  DropdownButtonFormField<Group>(
                    ...
                    items: _groups.map((group) {
                      return DropdownMenuItem(
                        value: group,
                        child: Text('${group.code} (${group.groupType.label})'),
                      );
                    }).toList(),
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            const Divider(),
                            _buildInfoRow(Icons.tag, 'Kod', _selectedGroup!.code),
                            _buildInfoRow(Icons.school, 'Kierunek', _selectedGroup!.program.name),
                            _buildInfoRow(Icons.calendar_today, 'Rok naboru', _selectedGroup!.year.year.toString()),
                            _buildInfoRow(Icons.stars, 'Specjalizacja', _selectedGroup!.specialization.name),
                            _buildInfoRow(Icons.category, 'Typ zajęć', _selectedGroup!.groupType.label),
                          ],
                        ),
                      ),
                    ),
